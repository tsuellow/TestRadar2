...
import com.example.android.testradar2.vectorLayerUtils.BarriosLayer;
import com.example.android.testradar2.vectorLayerUtils.GeoJsonUtils;
...
public class GettingStartedMarkerFused extends Activity implements GoogleApiClient.ConnectionCallbacks,
    ...
    ItemizedLayer<OtherTaxiMarker> mOtherTaxisLayer;
...
//            }
            ...
            locationCallback = new LocationCallback() {
                ...
                public void onLocationResult(LocationResult locationResult) {
                    ...
                    mWebSocketConnection.attemptSend(new TaxiObject(new TaxiMarker(Constants.myId,endLocation,mCompass.getRotation(),"taxi",0)).convertToJson());
        ...
        final Drawable otherIcon=getResources().getDrawable(R.drawable.frame01);
        otherIcon.setTint(Color.BLUE);
        Drawable grayedIcon= otherIcon.getConstantState().newDrawable().mutate();
        ...
        Drawable baseIcon=getResources().getDrawable(R.drawable.frame01);
        final Drawable clickedIcon= baseIcon.getConstantState().newDrawable().mutate();
        clickedIcon.setTint(Color.BLACK);
        ItemizedLayer.OnItemGestureListener<OtherTaxiMarker> listener= new ItemizedLayer.OnItemGestureListener<OtherTaxiMarker>() {
            @Override
            public boolean onItemSingleTapUp(int index, OtherTaxiMarker item) {
...
//                    currSize=0;
//                }
//                if (clickedItems.contains((Integer)item.getTaxiId())){
//                    clickedItems.remove((Integer)item.getTaxiId());
//                    item.setRotatedMarker(new MarkerSymbol(AndroidGraphicsCustom.drawableToBitmap(otherIcon,101+currSize),HotspotPlace.CENTER,false));
//                    Toast.makeText(mContext, "is contained", Toast.LENGTH_LONG).show();
//                }else{
//                    clickedItems.add(item.getTaxiId());
//                    item.setRotatedMarker(new MarkerSymbol(AndroidGraphicsCustom.drawableToBitmap(clickedIcon,101+currSize),HotspotPlace.CENTER,false));
//                    Toast.makeText(mContext, "is not contained", Toast.LENGTH_LONG).show();
//                }
//                mOtherTaxisLayer.update();
//                mapView.map().updateMap(true);

                Toast.makeText(mContext, item.getTaxiId() + " id", Toast.LENGTH_LONG).show();
                return false;
            }

            @Override
            public boolean onItemLongPress(int index, OtherTaxiMarker item) {
                return false;
            }
        };
        ...
        otherSymbol = new MarkerSymbol(otherBitmap, HotspotPlace.CENTER, false);

        mMarkerLayer = new ItemizedLayer<TaxiMarker>(mapView.map(), new ArrayList<TaxiMarker>(), symbol, this);
        mOtherTaxisLayer=new ItemizedLayer<OtherTaxiMarker>(mapView.map(),otherSymbol);
...
//            e.printStackTrace();
//        }
        List<OtherTaxiMarker> taxiMarkerOthers=new ArrayList<OtherTaxiMarker>();
...
//        OtherTaxiMarker taxiMarker1=new OtherTaxiMarker(1,13.0925,-86.36089);
//        OtherTaxiMarker taxiMarker2=new OtherTaxiMarker(2,13.0921,-86.360);
//        OtherTaxiMarker taxiMarker3=new OtherTaxiMarker(3,13.092,-86.361);
//        taxiMarker1.setMarker(otherSymbol);
//        taxiMarker2.setMarker(otherSymbol);
//        taxiMarker3.setMarker(otherSymbol);
//        taxiMarkerOthers.add(taxiMarker1);
//        taxiMarkerOthers.add(taxiMarker2);
//        taxiMarkerOthers.add(taxiMarker3);
//        mOtherTaxisLayer.addItem(taxiMarkerOthers.get(0));
//        mOtherTaxisLayer.addItem(taxiMarkerOthers.get(1));
//        mOtherTaxisLayer.addItem(taxiMarkerOthers.get(2));
        //mOtherTaxisLayer.removeItem(2);



        prepareCompassRecalibrateDialog();

        mAnimationEngine=new AnimationEngine(mapView.map(),otherIcon,19);

//        mAnimationEngine.setAnimationDataListener(new AnimationEngine.AnimationDataListener() {
//            @Override
//            public void onAnimationLayersReady(final ArrayList<OtherTaxiMarker>[] animationFrames) {
...
//                            double currZoom=mapView.map().getMapPosition().getZoom();
//                            int currSize;
//                            if (currZoom>16 && currZoom<17){
//                                currSize=(int) (100*(currZoom-16));
//                            }else if (currZoom>=17){
//                                currSize=99;
//                            }else{
//                                currSize=0;
//                            }
//                            if (mAnimationEngine.frameZoom!=currZoom){
//
//                                for(OtherTaxiMarker otherTaxiMarker:animationFrames[i]){
//                                    otherTaxiMarker.setRotatedMarker(new MarkerSymbol(mOtherScaleSymbols[currSize],HotspotPlace.CENTER,false));
//                                }
//                            }
//                            if (clickedItems.size()!=0){
//                                for(OtherTaxiMarker otherTaxiMarker:animationFrames[i]){
//                                    for (Integer taxiId:clickedItems){
//                                        if (taxiId.equals((Integer)otherTaxiMarker.getTaxiId())){
//                                            otherTaxiMarker.setRotatedMarker(new MarkerSymbol(AndroidGraphicsCustom.drawableToBitmap(clickedIcon,101+currSize),HotspotPlace.CENTER,false));
//                                        }
//                                    }
//                                }
//                            }
//                            mOtherTaxisLayer.addItems(animationFrames[i]);
//                            mOtherTaxisLayer.update();
//                            double test=0.0;
//                            if(mOtherTaxisLayer.getItemList().size()>0){
//                                test=mOtherTaxisLayer.getItemList().get(0).getRotation();
//                            }
//                            Log.d("output other"+i,test+"");
...
//        });
        ...
        mWebSocketConnection=new WebSocketConnection("https://id-ex-theos-taxi-test1.herokuapp.com/",this, mContext,mAnimationEngine);
        ...
        mWebSocketConnection.setAnimationDataListener(new WebSocketConnection.AnimationDataListener() {
            ...
            public void onAnimationParametersReceived(List<TaxiObject> baseTaxis, List<TaxiObject> newTaxis) {
                ...
                if (baseTaxis.size()>0) {
                    for (int i = 0; i < baseTaxis.size(); i++) {
                        ...
                        if (taxiObject.getIsActive() == 0) {
                            mOtherTaxisLayer.getItemList().get(i).setPurpose(OtherTaxiMarker.Purpose.DISAPPEAR);
                        } else {
                            mOtherTaxisLayer.getItemList().get(i).setPurpose(OtherTaxiMarker.Purpose.MOVE);
                        }
                ...
                for (TaxiObject tO:newTaxis){
                    OtherTaxiMarker otherTaxiMarker=new OtherTaxiMarker(tO);
                    otherTaxiMarker.setPurpose(OtherTaxiMarker.Purpose.APPEAR);
                    otherTaxiMarker.setPurposeTaxiObject(tO);
                    otherTaxiMarker.setRotatedMarker(otherSymbol);
                    mOtherTaxisLayer.addItem(otherTaxiMarker);
                }
                if (mOtherTaxisLayer.getItemList().size()>0)
                Log.d("shownt",mOtherTaxisLayer.getItemList().get(0).getLatitude()+"");
    ...
    public void setOffAnimation(final int frames){
        for (int i=0;i<frames;i++) {
            ...
            mapView.postDelayed(new Runnable() {
                @Override
                public void run() {
                    for (OtherTaxiMarker marker : mOtherTaxisLayer.getItemList()) {
                        marker.executeFrame(a,frames);
                    }
                    mOtherTaxisLayer.populate();
                    mOtherTaxisLayer.update();
                    mapView.map().updateMap(false);
                }
            }, i*500 / frames);
...
//    @Override
//    public void onAnimationLayersReady(final ItemizedLayer<TaxiMarker>[] animationFrames) {
//        for (int a = 0; a<animationFrames.length ;a++) {
//            final int i=a;
//            mapView.postDelayed(new Runnable() {
//                @Override
//                public void run() {
//                    mOtherTaxisLayer=animationFrames[i];
//                    mapView.map().updateMap(true);
//                }
//            }, 25 * a);
//        }
...
//            }
    ...
    public static MarkerSymbol retrieveScaledGrayedSymbol(Bitmap[] symbolArray, double zoom){
        int currSize;
        if (zoom>16 && zoom<17){
            currSize=(int) (100*(zoom-16));
        }else if (zoom>=17){
        ...
        return new MarkerSymbol(symbolArray[currSize],HotspotPlace.CENTER,false);
    }
    ...
    public void scaleOtherIcons(int scale){
        ...
        for (OtherTaxiMarker otherTaxiMarker:mOtherTaxisLayer.getItemList()){
            otherTaxiMarker.setRotatedMarker(new MarkerSymbol(otherSymbol,HotspotPlace.CENTER,false));
        }
        mapView.map().updateMap(false);
    }

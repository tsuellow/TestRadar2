...
import com.example.android.testradar2.deprecated.PositionObject;
                ...
                (Request.Method.POST, "https://www.awicon.de/theostaxi/php/gettest.php", null, new Response.Listener<JSONArray>() {
                    ...
                    public void onResponse(JSONArray response) {
                        try {
                            ...
                            for(int i=0; i<response.length(); i++){
                                JSONObject jresponse = response.getJSONObject(i);
                                taxiMarkers[i]=new TaxiMarker(jresponse.getInt("userid"),jresponse.getDouble("lat"),jresponse.getDouble("lon"));
